...
app.add_middleware(
    ...
    allow_origin_regex=r"^(https?://(localhost|127\.0\.0\.1|192\.168\.\d{1,3}\.\d{1,3})(:\d+)?|https://.*\.vercel\.app)$",
...
def sanitize_chat_result(result: dict, query: str) -> dict:
    """Preserve genuine RAG-grounded replies from the chat agent.

    Only rewrite the reply when it matches the *old* inert fallback markers
    ("matching product", "PDF technical report"). Real grounded answers that
    legitimately contain prices (AED), URLs, or numbered lists must be kept,
    and retrieved `sources` must be preserved so the frontend can show them.
    """
    result = dict(result)
    reply = str(result.get("reply", ""))
    # Only rewrite when the agent returned the legacy inert template. Detect that
    # template by its exact signature rather than a loose substring, because real
    # RAG answers legitimately contain words like "matching product" / "PDF report".
    looks_like_old_fallback = (
        "MARKET RESULT is ready for matched options" in reply
        or "Next Details Needed" in reply
    )
    if looks_like_old_fallback:
        result["reply"] = natural_no_match_reply(query)
        result["needs_clarification"] = False
        result["questions"] = []
    result["recommendation"] = None
    # Keep retrieved sources so the UI can display RAG provenance.
